...
public class Graph {
     ...
     public int calcPathDuration(ArrayList<Activity> arrayList) {
     	int total = 0;
     	if(arrayList != null) {
 	    	for(int x = 0; x < arrayList.size(); x++) {
 	    		total += arrayList.get(x).getDuration();
 	    	}
     	}
     	return total;
     }
     ...
     public void InsertionSort() {
 		for(int i = 1; i < allPaths.size(); i++) {
 			ArrayList<Activity> key = allPaths.get(i);
 			int j = i-1;
 			
 			while(j >= 0 && calcPathDuration(allPaths.get(j)) > calcPathDuration(key)) {
 				allPaths.set(j+1, allPaths.get(j));
 				j--;
 			}
 			allPaths.set(j+1, key);
